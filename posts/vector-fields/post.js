...
var FIELDS = [
  { key: 'uniform', label: 'یکنواخت',
    ...
    fml: 'F = (0.85, 0.3)',
    note: 'همه‌جا یک فلش، با یک اندازه و یک جهت. نه چیزی می‌جوشد، نه چیزی می‌چرخد. کسل‌کننده‌ترین میدان ممکن — و دقیقاً به همین دلیل، مبنای مقایسه.' },
  { key: 'source', label: 'سرچشمه',
    f: function (x, y) { return [x, y]; },
    fml: 'F = (x, y)',
    note: 'همه‌چیز از مرکز فرار می‌کند و هرچه دورتر، تندتر. این یعنی نه‌فقط مرکز، بلکه <b>هر</b> نقطه‌ای از این میدان دارد چیزی تولید می‌کند: دیورژانس همه‌جا برابر ۲ است.' },
  { key: 'sink', label: 'چاه',
    f: function (x, y) { return [-x, -y]; },
    fml: 'F = (−x, −y)',
    note: 'همان قبلی با علامت وارونه. همه‌جا دارد بلعیده می‌شود، پس دیورژانس همه‌جا منفی است.' },
  { key: 'vortex', label: 'چرخش',
    f: function (x, y) { return [-y, x]; },
    fml: 'F = (−y, x)',
    note: 'چرخش یک جسم صُلب — مثل نقطه‌ای روی صفحهٔ گرام‌فون. هیچ‌جا چیزی تولید نمی‌شود (دیورژانس صفر) ولی همه‌جا می‌چرخد (کرل برابر ۲، در تمام صفحه، نه فقط وسط).' },
  { key: 'charge', label: 'بار نقطه‌ای',
    f: function (x, y) {
      var r2 = x * x + y * y + SOFT;
      return [x / r2 * 0.8, y / r2 * 0.8];
    },
    fml: 'F = r̂ / r²',
    note: 'میدان الکتریکیِ یک بار نقطه‌ای. با فاصله دقیقاً آن‌قدر ضعیف می‌شود که دیورژانسش <b>همه‌جا صفر</b> بماند — جز خودِ نقطهٔ مرکز، که آنجا بی‌نهایت است. کل قانون گاوس همین جمله است.' },
  { key: 'wire', label: 'گردابِ سیم',
    ...
    fml: 'F = θ̂ / r',
    note: 'میدان مغناطیسیِ دور یک سیمِ حامل جریان. خطوطش دایره‌های کاملند، ولی <b>کرلش همه‌جا صفر است</b> جز روی خود سیم. اینکه میدان دور می‌زند، به این معنی نیست که کرل دارد.' },
  { key: 'shear', label: 'برشی',
    f: function (x, y) { return [y * 0.9, 0]; },
    fml: 'F = (0.9y, 0)',
    note: 'جریان کاملاً مستقیم است — هیچ‌جا هیچ چیزی دور نمی‌زند — ولی بالا تندتر از پایین می‌رود. کرل همه‌جا مخالف صفر است. این مهم‌ترین مثال کل مقاله است.' },
  { key: 'saddle', label: 'زین',
    f: function (x, y) { return [x, -y]; },
    fml: 'F = (x, −y)',
    note: 'از دو طرف فشرده و از دو طرف کشیده می‌شود. هرچه از یک طرف وارد می‌شود از طرف دیگر بیرون می‌رود: هم دیورژانس صفر، هم کرل صفر. با این حال میدانِ کاملاً بی‌خاصیتی نیست.' }
];
...
(function () {
  ...
  var f = Fig(cv, 1.12, function (fig) {
    ...
    ro(out, [
      ['فرمول', cur.fml],
      ['<span class="q">∇·F</span> در یک نقطهٔ دلخواه', say(d)],
      ['<span class="q">∇×F</span> در همان نقطه', say(c)]
    ]);
...
(function () {
  ...
  var f = Fig(cv, 1.12, function (fig) {
    ...
    var msg;
    if (Math.abs(Φ) < 0.02) {
      msg = 'شار صفر است: هرچه وارد حلقه می‌شود، دقیقاً همان‌قدر هم خارج می‌شود. اینجا نه شیری هست نه چاهی — جریان فقط دارد رد می‌شود.';
    } else if (Φ > 0) {
      msg = 'شار مثبت است: بیشتر از آنچه وارد شده خارج می‌شود، پس داخل این حلقه چیزی دارد <b>تولید</b> می‌شود. یک شیر آب.';
    } else {
      msg = 'شار منفی است: کمتر از آنچه وارد شده خارج می‌شود، پس داخل حلقه چیزی دارد <b>بلعیده</b> می‌شود. یک چاه.';
    }
    if (cur.key === 'charge') {
      msg += Math.hypot(P.x, P.y) < R
        ? ' — و این‌بار بار نقطه‌ای <b>داخل</b> حلقه است.'
        : ' — بار نقطه‌ای بیرون حلقه است؛ حلقه را ببر رویش.';
    }
    note.innerHTML = msg;
  });
...
(function () {
  ...
  var f = Fig(cv, 1.12, function (fig) {
    ...
    if (Math.abs(exact) < 0.05) {
      msg = 'چرخونک بی‌حرکت است: هرچه از یک طرف هلش می‌دهد، از طرف دیگر همان‌قدر برش می‌گرداند. کرل صفر.';
    } else {
...
(function () {
  ...
  var f = Fig(cv, 1.12, function (fig) {
    ...
    ro(out, [
      ...
      ['میدان', a >= 0 ? (b >= 0 ? 'سرچشمه + پادساعتگرد' : 'سرچشمه + ساعتگرد')
                       : (b >= 0 ? 'چاه + پادساعتگرد' : 'چاه + ساعتگرد')]
    ]);

    var msg;
    if (Math.abs(a) < 0.06 && Math.abs(b) < 0.06) msg = 'هر دو صفر: هیچ فلشی نمانده. میدانِ هیچ.';
    else if (Math.abs(b) < 0.06) msg = 'فقط دیورژانس. خطوط، شعاع‌های صافند — نه می‌پیچند، نه دور می‌زنند.';
    else if (Math.abs(a) < 0.06) msg = 'فقط کرل. خطوط، دایره‌های بسته‌اند — هیچ ذره‌ای هیچ‌وقت از مرکز دور یا به مرکز نزدیک نمی‌شود.';
    else msg = 'هر دو با هم: مارپیچ. و باز هم هرکدام سرِ کار خودش است — لغزندهٔ بالا فقط دیورژانس را عوض می‌کند و پایینی فقط کرل را.';
    note.innerHTML = msg;
  });
