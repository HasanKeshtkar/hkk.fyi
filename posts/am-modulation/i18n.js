...
(function (global) {
  ...
  en: {
    ...
    "start.li1": "Sound cannot be sent as sound. At speech frequencies a good antenna would be <b>tens of kilometres</b> long, and every station on Earth would sit in the same band, on top of each other.",
    ...
    "start.li2": "So we start from a fast, steady sine wave — the <b>carrier</b> — and let the message control its height: <b>s(t) = A<sub>c</sub>[1 + m·x(t)]·cos(2πf<sub>c</sub>t)</b>.",
    "start.li3": "The shape of the message then appears as the <b>envelope</b> of the transmitted wave. That is the whole trick.",
    "start.li4": "Keep the modulation index <b>m ≤ 1</b>. Above 1 the envelope folds over and the message cannot be recovered cleanly.",
    "start.li5": "In frequency, the message moves up and sits next to the carrier as two mirrored <b>sidebands</b>. AM uses <b>twice</b> the bandwidth of the message.",
    "start.li6": "The carrier itself carries no information, yet takes at least two thirds of the power. AM pays that price to make the receiver a <b>diode, a resistor and a capacitor</b>.",
    ...
    "start.monolabel3": "THE WHOLE ARTICLE IN 30 SECONDS",
    "start.monolabel4": "CONTENTS",
    "start.p1": "A microphone gives you a signal that swings a few thousand times a second. A radio wave that can cross a city swings a few million times a second. Modulation is how you get the first one to travel on the second one. <br><br> This article is about the simplest method, <b>amplitude modulation</b>. It is the right place to start, because every other method is a variation on the same idea: take a steady high-frequency wave, and let the message change something about it. <br><br> You only need to know what a sine wave is. <b>Every figure below is live</b> — drag it, break it, watch what happens.",
    ...
    "start.p3": "Still here? Good — the rest is the reasoning, plus seven things you can drag.",
  ...
  fa: {
    ...
    "start.li1": "صدا را نمی‌شود به شکل صدا فرستاد. در فرکانس‌های گفتار، آنتنِ درست‌وحسابی <b>ده‌ها کیلومتر</b> طول می‌خواهد؛ و تازه اگر هم می‌شد، همه‌ی فرستنده‌های دنیا در یک باند و روی هم می‌افتادند.",
    ...
    "start.li2": "پس از یک موج سینوسیِ تند و یکنواخت شروع می‌کنیم — <b>حامل</b> — و می‌گذاریم پیام ارتفاعش را کنترل کند: <b>s(t) = A<sub>c</sub>[1 + m·x(t)]·cos(2πf<sub>c</sub>t)</b>.",
    "start.li3": "آن‌وقت شکل پیام به‌صورت <b>پوشِ</b> موج فرستاده‌شده ظاهر می‌شود. کل ترفند همین است.",
    "start.li4": "شاخص مدولاسیون را زیر یک نگه دارید، <b>m ≤ 1</b>. بالاتر از یک، پوش از صفر رد می‌شود و تا می‌خورد، و دیگر نمی‌شود پیام را سالم بیرون کشید.",
    "start.li5": "در فرکانس، پیام بالا می‌رود و به‌صورت دو <b>باند کناریِ</b> قرینه کنار حامل می‌نشیند. یعنی AM <b>دو برابرِ</b> پهنای باند پیام جا می‌گیرد.",
    "start.li6": "خودِ حامل هیچ اطلاعاتی حمل نمی‌کند، ولی دست‌کم دوسوم توان را می‌برد. AM این هزینه را می‌دهد تا گیرنده‌اش فقط <b>یک دیود، یک مقاومت و یک خازن</b> باشد.",
    ...
    "start.monolabel3": "کل مقاله در ۳۰ ثانیه",
    "start.monolabel4": "فهرست",
    "start.p1": "میکروفون سیگنالی به شما می‌دهد که چند هزار بار در ثانیه بالا و پایین می‌رود. موجی که می‌تواند از یک شهر عبور کند، چند میلیون بار در ثانیه نوسان می‌کند. مدولاسیون همان کاری است که اولی را سوار دومی می‌کند.<br><br>این نوشته درباره‌ی ساده‌ترین راهِ این کار است: <b>مدولاسیون دامنه</b>. شروع از همین‌جا درست است، چون هر روش دیگری در نهایت شکل دیگری از همین یک فکر است — یک موج پرفرکانس و یکنواخت بردارید، و بگذارید پیام چیزی از آن را عوض کند.<br><br>فقط کافی است بدانید موج سینوسی چیست. <b>هر شکلی که پایین‌تر می‌بینید زنده است</b> — بکشیدش، خرابش کنید، ببینید چه می‌شود.",
    ...
    "start.p3": "هنوز اینجایید؟ خوب است — بقیه‌اش استدلال همین‌هاست، به‌اضافه‌ی هفت چیز که می‌شود دستکاری‌شان کرد.",
